...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if (user_type == 'customer') 
        {
            ...
            return ['emergency_jobs' => $emergency_jobs, 'normal_jobs', 'jobs' => $jobs, 'user' => $user, 'user_type' => $user_type, 'num_pages' => 0, 'pagenum' => 0];
        } 
        elseif (user_type == 'translator') 
        {
            $jobs_ids = Job::getTranslatorJobsHistoric($user->id, 'historic', $pagenum);
            $totaljobs = $jobs_ids->total();
            $num_pages = ceil($totaljobs / 15);
            $jobs = $jobs_ids;
            $normal_jobs = $jobs_ids;
            //    $jobs['data'] = $normal_jobs;
            //    $jobs['total'] = $totaljobs;
            return ['emergency_jobs' => $emergency_jobs, 'normal_jobs' => $normal_jobs, 'jobs' => $jobs, 'user' => $user, 'user_type' => $user_type, 'num_pages' => $num_pages, 'pagenum' => $pagenum];
        }
    }
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) 
        {
            ...
            if (isset($data->customer_phone_type)) 
            {
                ...
                $response->customer_phone_type = true; 
            }
            else 
            {
                $data->customer_phone_type = false;
                $response->customer_phone_type = false; 
            }
            ...
            $data->by_admin = isset($data->by_admin) ? true : false;
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
     ...
     * @return mixed
     */
    public function storeJobEmail($data, $user)
    {
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $data->language = TeHelper::fetchLanguageFromJob_id($data->from_language_id);
        ...
        $this->sendPushNotificationToSpecificUers($translator_array, $job->id, $data, $msg_text, false);       // send new booking push to suitable translators(not delay)
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $phoneJobMessageTemplate = trans('sms.phone_job', ['date' => $date, 'time' => $time, 'duration' => $duration, 'job_id' => $job_id]);

        $physicalJobMessageTemplate = trans('sms.physical_job', ['date' => $date, 'time' => $time, 'town' => $city, 'duration' => $duration, 'job_id' => $job_id]);
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        if (env('APP_ENV') == 'prod') 
        {
            $one_signal_app_id = config('app.prodOnesignalAppID');
            $one_signal_rest_auth_key = sprintf("Authorization: Basic %s", config('app.prodOnesignalApiKey'));
        } 
        else 
        {
            $one_signal_app_id = config('app.devOnesignalAppID');
            $one_signal_rest_auth_key = sprintf("Authorization: Basic %s", config('app.devOnesignalApiKey'));
        }
    ...
    public function updateJob($id, $data, $user)
    {
        ...
        {
            $log_data[] = [
                'old_lang' => TeHelper::fetchLanguageFromJob_id($job->from_language_id),
                'new_lang' => TeHelper::fetchLanguageFromJob_id($data->from_language_id)
            ];
    ...
    private function changeTimedoutStatus($job, $data, $changed_translator)
    {
        ...
        if ($data->status == 'pending') 
        {
            ...
            $subject = 'Vi har nu återöppnat er bokning av ' . TeHelper::fetchLanguageFromJob_id($job->from_language_id) . 'tolk för bokning #' . $job->id;
    ...
    private function changePendingStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data->status == 'assigned' && $changedTranslator) 
        {
            ...
            $language = TeHelper::fetchLanguageFromJob_id($job->from_language_id);
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($user && $user->user_type == env('SUPERADMIN_ROLE_ID')) 
        {
            ...
            if (!empty(isset($request->feedback))) 
            {
                ...
                if (isset($request->count)) return ['count' => $all_jobs->count()];
            }

            if (!empty(isset($request->id))) 
            {
                if (is_array($request->id))
                    $all_jobs->whereIn('id', $request->id);
                else
                    $all_jobs->where('id', $request->id);
                $request = array_only($request, ['id']);
            }

            if (!empty(isset($request->lang))) 
            {
                $all_jobs->whereIn('from_language_id', $request->lang);
            }
            if (!empty(isset($request->status))) 
            {
                $all_jobs->whereIn('status', $request->status);
            }
            if (!empty(isset($request->expired_at))) 
            {
                $all_jobs->where('expired_at', '>=', $request->expired_at);
            }
            if (!empty(isset($request->will_expire_at))) 
            {
                $all_jobs->where('will_expire_at', '>=', $request->will_expire_at);
            }
            if (!empty(isset($request->customer_email))) 
            {
            ...
            if (!empty(isset($request->translator_email))) {
                $users = DB::table('users')->whereIn('email', $request->translator_email)->get();
                if ($users) 
                {
                    $allJob_iDs = DB::table('translator_job_rel')->whereNull('cancel_at')->whereIn('user_id', collect($users)->pluck('id')->all())->lists('job_id');
                    $all_jobs->whereIn('id', $allJob_iDs);
                }
            }
            if (isset($request->filter_timetype) && $request->filter_timetype == "created") 
            {
                if (!empty(isset($request->from))) 
                {
                    $all_jobs->where('created_at', '>=', $request["from"]);
                }
                if (!empty(isset($request->to))) 
                {
            ...
            if (isset($request->filter_timetype) && $request->filter_timetype == "due") 
            {
                if (!empty(isset($request->from))) 
                {
                    $all_jobs->where('due', '>=', $request["from"]);
                }
                if (!empty(isset($request->to))) 
                {
                    $to = $request["to"] . " 23:59:00";
            ...
            if (!empty(isset($request->job_type))) 
            {
                $all_jobs->whereIn('job_type', $request->job_type);
                // $all_jobs->where('jobs.job_type', '=', $request->job_type);
            }

            if (isset($request->physical)) 
            {
                $all_jobs->where('customer_physical_type', $request->physical);
                $all_jobs->where('ignore_physical', 0);
            }

            if (isset($request->phone)) 
            {
                $all_jobs->where('customer_phone_type', $request->phone);
                if(isset($request->physical))
            ...
            {
                $all_jobs->where('flagged', $request->flagged);
                $all_jobs->where('ignore_flagged', 0);
            }

            if (empty(isset($request->distance))) 
            {
            ...
            if (!empty(isset($request->consumer_type))) 
            {
                $all_jobs->whereHas('user.userMeta', function($q) use ($request) {
    ...
    public function alerts()
    {
        ...
        if ($user && $user->is('superadmin')) 
        {
            ...
            {
                $all_jobs->whereIn('jobs.from_language_id', $request->lang)
                    ->where('jobs.ignore', 0);
                /*$all_jobs->where('jobs.from_language_id', '=', $request->lang);*/
            }
            if (isset($request->status) && $request->status != '') 
            {
                $all_jobs->whereIn('jobs.status', $request->status)
                    ->where('jobs.ignore', 0);
                /*$all_jobs->where('jobs.status', '=', $request->status);*/
            }
            if (isset($request->customer_email) && $request->customer_email != '') 
            {
                $user = DB::table('users')->where('email', $request->customer_email)->first();
                
                $all_jobs->where('jobs.user_id', '=', $user->id)->where('jobs.ignore', 0);
            ...
            if (isset($request->translator_email) && $request->translator_email != '') 
            {
                ...
                $allJob_iDs = DB::table('translator_job_rel')->where('user_id', $user->id)->lists('job_id');
                $all_jobs->whereIn('jobs.id', $allJob_iDs)->where('jobs.ignore', 0);
            ...
            if (isset($request->filter_timetype) && $request->filter_timetype == "created") 
            {
                if (isset($request->from) && $request->from != "") 
                {
                    $all_jobs->where('jobs.created_at', '>=', $request["from"])->where('jobs.ignore', 0);
                }
                if (isset($request->to) && $request->to != "") 
                {
                    $to = $request["to"] . " 23:59:00";
            ...
            if (isset($request->filter_timetype) && $request->filter_timetype == "due") 
            {
                if (isset($request->from) && $request->from != "") 
                {
                    $all_jobs->where('jobs.due', '>=', $request["from"])
                        ->where('jobs.ignore', 0);
                }
                if (isset($request->to) && $request->to != "") 
                {
            ...
            {
                $all_jobs->whereIn('jobs.job_type', $request->job_type)
                    ->where('jobs.ignore', 0);
                /*$all_jobs->where('jobs.job_type', '=', $request->job_type);*/
            }
        ...
        return ['all_jobs' => $all_jobs, 'languages' => $languages, 'all_customers' => $all_customers, 'all_translators' => $all_translators, 'request' => $request];
    }
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        if ($user && ($user->is('superadmin') || $user->is('admin'))) 
        {
            ...
            if (isset($request->translator_email) && $request->translator_email != '') 
            {
                ...
                $allJob_iDs = DB::table('translator_job_rel')->where('user_id', $user->id)->lists('job_id');
                $all_jobs->whereIn('jobs.id', $allJob_iDs)
                    ->where('jobs.status', 'pending')
                    ->where('jobs.ignore_expired', 0)
                    ->where('jobs.due', '>=', Carbon::now());
            ...
            if (isset($request->filter_timetype) && $request->filter_timetype == "created") 
            {
                if (isset($request->from) && $request->from != "") 
                {
                    $all_jobs->where('jobs.created_at', '>=', $request["from"])
                        ->where('jobs.status', 'pending')
                        ->where('jobs.ignore_expired', 0)
                        ->where('jobs.due', '>=', Carbon::now());
                }
    ...
    public function reopen($request)
    {
        ...
        //$data_reopen['updated_at'] = date('Y-m-d H:i:s');

        //        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $job_id);
        ...
        {
            $this->sendNotificationByAdminCancelJob($new_job_id);
